...
public class EnrollmentDAO implements DAOFactory<Enrollment> {
    ...
    public void updateEnrollmentToken(Enrollment enrollment) throws SQLException {
        String sql = "UPDATE adtg.enrollment SET tokens_avail = ? WHERE c_subject = ? AND c_number = ? AND sec_id = ? AND student_netid = ?";
        try (Connection connection = getConnection();
             PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, enrollment.getTokensAvail());
            ps.setString(2, enrollment.getSection().getCourse().getSubject());
            ps.setInt(3, enrollment.getSection().getCourse().getNumber());
            ps.setInt(4, enrollment.getSection().getSectionId());
            ps.setString(5, enrollment.getStudentNetId());
            ps.executeUpdate();
        }
    }
    

    public List<String> listEnrolledStudents(Course course) throws SQLException {
        String sql = "SELECT student_netid FROM adtg.enrollment WHERE c_subject = ? AND c_number = ?";
        ...
        try (Connection connection = getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    enrolledStudents.add(rs.getString("student_netid"));
                }
            }
        } 
